...
function mlEncode(s) {if(!s)return "";return s.replace(/\%/g,"%25").replace(/\n/g,"%0a").replace(/\r/g,"%0d").replace(/\t/g,"%09");}
function mlDecode(s) {if(!s)return "";return decodeURIComponent(s);}
function uscEncode(s) {if(!s)return "";return (""+s).replace(/_/g,"_u").replace(/'/g,"_a").replace(/"/g,"_q").replace(/(\\)/g,"_b");}
function uscDecode(s) {if(!s)return "";return (""+s).replace(/_b/g,"\\").replace(/_q/g,'"').replace(/_a/g,"'").replace(/_u/g,"_");}
...
function repeatMarkup(htmlStringOrID,arrayToUse) {
	...
	for(var i in a) {
		...
		if(typeof(e.literalHTML)=='string') {out+=e.literalHTML;continue;}
		e["#"] = nr++;
...
function win(s) {
	var windowsStylePath = s.replace(/(\/)/g,"\\");
...
function bindActions() {																								//#documentThis
	...
	$(document).on("dblclick","[data-action-dbl]",function() {

		var t = $(this);
		var d = t.hasClass("disabled"); if(d) return;	// disabled
		var x = t.data("action-dbl");
		if(typeof(Actions)    ==="undefined") return;	// no "actions" object defined
		if(typeof(Actions[x]) ==="undefined") return;	// no handler for this particular action
		Actions[x](t);
